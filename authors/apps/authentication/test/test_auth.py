...
class UserAuthenticationTestCase(BaseTest):
    def test_user_can_register(self):
        ...
        response = self.client.post(
            reverse("authentication:user_signup"), self.user_cred, format="json"
        )
    ...
    def test_password_reset_with_no_email(self):
        response = self.client.post(self.RESET_PASS, self.no_email, format="json")
        expected = {
    ...
    def test_password_reset_with_wrong_email(self):
        response = self.client.post(self.RESET_PASS, self.wrong_email, format="json")
        expected = {
            "message": "Password reset failed. Please check your email and try again"
        }
        self.assertEquals(response.data, expected)
    ...
    def test_password_reset_with_correct_email(self):
        ...
        response = self.client.post(self.RESET_PASS, self.correct_email, format="json")
