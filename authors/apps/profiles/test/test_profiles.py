...
class ProfileTestCase(BaseTest):
    image = {
        "user": {"image": "https://vsco.co/tichtoo/media/544f6752726708181b8b4a76"}
    }
    bio = {
        "user": {
    ...
    def update_user_bio(self, token, bio):
        ...
        return self.client.put(
            "/api/user/", bio, HTTP_AUTHORIZATION="Bearer " + token, format="json"
        )

    def update_user_image(self, token, image):
        """Helper method to update an user image"""
        return self.client.put(
            "/api/user/", image, HTTP_AUTHORIZATION="Bearer " + token, format="json"
        )
