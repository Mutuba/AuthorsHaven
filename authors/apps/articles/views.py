...
class ArticleViewSet(
    ...
    def create(self, request):
        ...
        serializer_context = {"author": request.user, "request": request}
    ...
    def list(self, request):
        ...
        # use self.get_queryset() to avoid cache results
...
class LikeAPIView(UpdateAPIView):
    ...
    def update(self, request, slug):
        ...
        if user in article.dislike.all():
            ...
            return Response(
                {"message": "Removed from dislike and Added to Liked articles"},
                status=status.HTTP_200_OK,
            )
        else:
            ...
            article.like.add(user.id)
            return Response(
                {"message": "Added to Liked articles"}, status=status.HTTP_200_OK
            )
...
class DisLikeAPIView(UpdateAPIView):
    ...
    def update(self, request, slug):
        ...
        if user in article.like.all():
            ...
            return Response(
                {
                    "message": "Removed from Liked Articles and Added to Disliked articles"
                },
                status=status.HTTP_200_OK,
            )
        else:
            ...
            article.dislike.add(user.id)
            return Response(
                {"message": "You Dislike this Article"}, status=status.HTTP_200_OK
            )
...
class LikeComment(APIView):
    ...
    def put(self, request, **kwargs):
        ...
        if request.user in comment.likes.all():
            ...
            return Response(
                {"message": "You unliked this article."}, status=status.HTTP_200_OK
            )

        comment.likes.add(request.user)
        return Response(
            {"message": "You liked this comment"}, status=status.HTTP_200_OK
        )
...
class DislikeComment(APIView):
    ...
    def put(self, request, **kwargs):
        ...
        if request.user in comment.dislikes.all():
            comment.dislikes.remove(request.user)
            return Response(
                {"message": "You undisliked this article."}, status=status.HTTP_200_OK
            )

        comment.dislikes.add(request.user)
        return Response(
            {"message": "You disliked this comment"}, status=status.HTTP_200_OK
        )
...
class RateArticlesAPIView(APIView):
    ...
    def post(self, request, slug):
        ...
        if currentRating is None:
            article_rating = ArticleRating(
                article=article, rating=rating, rater=user, note=note
            )
            article_rating.save()
        ## update the rating
        else:
        ...
        average = (
            ArticleRating.objects.filter(article=article)
            .aggregate(Avg("rating"))
            .get("rating__avg", 0)
        )
        return Response(
            {"average_rating": average, "Note": note}, status=status.HTTP_201_CREATED
        )
    ...
    def get(self, request, slug):
        ...
        # Return the rating
        if currentRating:
            return Response({"rating": currentRating.rating}, status=status.HTTP_200_OK)
