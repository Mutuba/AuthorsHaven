...
class ArticleSerializer(serializers.ModelSerializer):
    ...
    average_ratings = serializers.SerializerMethodField(method_name="average_count")
    ...
    def average_count(self, object):
        average = (
            ArticleRating.objects.filter(article=object)
...
class CommentSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        recipients = [
            User.objects.get(pk=profile.id)
            for profile in Profile.objects.all()
            if (profile.has_favorited(article) and profile.get_notifications)
        ]

        notify.send(
            author, recipient=recipients, verb="commented on", action_object=article
        )
        return comment
...
class RatingSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if isinstance(rate, str) or rate is None:
            raise serializers.ValidationError("""Only integers allowed.""")
